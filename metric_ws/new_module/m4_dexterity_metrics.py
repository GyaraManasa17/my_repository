...
class DexterityAnalyzer:
    ...
    def __init__(self, node, sampler, parser):
        ...
        # Flag to ensure we only print the numerical Jacobian warning once
        self._numerical_warning_printed = False
    ...
    def compute_jacobian(self, q, delta=1e-3):
        ...
        # NEW: Print a loud warning once if we fall back to the slow numerical method
        if not self._numerical_warning_printed:
            self.node.get_logger().warning("⚠️ Analytic Jacobian unavailable! Falling back to Numerical Central Difference.")
            self.node.get_logger().warning("⚠️ This requires 2*N FK service calls per point and will be significantly slower.")
            self._numerical_warning_printed = True
    ...
    # MAIN ANALYSIS
    # ------------------------------------------------
    # NEW: Added dataset_file parameter for explicit pipeline passing
    def analyze(self, dataset_file=None):
        try:
