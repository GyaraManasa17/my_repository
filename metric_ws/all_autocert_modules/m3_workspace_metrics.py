...
class WorkspaceAnalyzer:
    ...
    def __init__(self, node):
        ...
        self.distributions_dir = os.path.join(self.output_dir, "distributions") # NEW: For stat analysis
    ...
    def analyze(self):
        try:
            ...
            alpha_volume = np.mean(neighbor_dist)
            metrics["average_neighbor_distance"] = float(alpha_volume)
            ...
            # --- NEW: Save Workspace Statistics Distributions ---
            ...
            self.plot_density(df) # Serves as 2D Reachability Heatmap
            self.plot_radial_distribution(r)
            self.plot_polar_distribution(points)
            self.plot_voxel_occupancy_1d(points) # Original 1D histogram
            
            # --- NEW PLOTS ADDED BASED ON RECOMMENDATIONS ---
            self.plot_voxel_occupancy_3d(points)  # Full 3D Voxel Reachability Grid
            self.plot_workspace_slices(points)    # Z = Constant slices
            self.plot_cross_section(points)       # Radius vs Height cross-section
            
            self.plot_workspace_projections(points)
            self.plot_directional_histogram(points)
            self.plot_surface_mesh(points, hull) # Acts as Reachable Surface mesh
    ...
    def plot_voxel_occupancy_3d(self, points):
        ...
        # Calculate centers of voxels for plotting
    ...
    def plot_workspace_slices(self, points):
        ...
        for z in z_levels:
            # Mask points roughly within the current Z level slice (+/- 2cm)
...
#     #     self.node.get_logger().info(f"📂 Loading dataset {dataset_file}")
...
#     #     metrics["voxel_count"] = int(unique_voxels)
...
#     #     metrics["workspace_anisotropy"] = float(np.max(eigvals) / np.min(eigvals))
...
#     #     self.node.get_logger().info(f"📊 Metrics saved to {metrics_file}")
...
#                     "⚠️ Dataset CSV is empty. No analysis performed."
#                 )
#                 return None

#             if not {"x", "y", "z"}.issubset(df.columns):
#                 self.node.get_logger().error(
#                     "❌ Dataset missing required columns (x,y,z)."
#                 )
#                 return None
...
#             if len(df) == 0:
#                 self.node.get_logger().warning(
#                     "⚠️ No reachable FK points found after filtering."
#                 )
#                 return None
...
#                 f"📊 Analyzing {len(points)} reachable workspace points..."
#             )
...
#             self.node.get_logger().info("📈 Generating workspace plots...")
...
#             self.node.get_logger().error(
#                 f"❌ Workspace analysis failed: {str(e)}"
#             )
...
#         plt.title("Workspace Density XY")

#         plt.savefig(f"{self.plots_dir}/workspace_density_xy.png")

#         plt.close()

#         plt.hist2d(df["x"], df["z"], bins=50)

#         plt.title("Workspace Density XZ")

#         plt.savefig(f"{self.plots_dir}/workspace_density_xz.png")

#         plt.close()

#         plt.hist2d(df["y"], df["z"], bins=50)

#         plt.title("Workspace Density YZ")

#         plt.savefig(f"{self.plots_dir}/workspace_density_yz.png")
...
#         plt.scatter(theta, r, s=1)

#         plt.title("Polar Reachability Plot")
