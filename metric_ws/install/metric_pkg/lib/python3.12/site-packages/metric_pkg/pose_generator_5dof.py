...
class DatasetGenerator(Node):
    ...
    def generate_dataset(self, num_trials=1000, seed=42):
        ...
        # 5DOF joint names from URDF (always use 5DOF model)
        joint_names = [
            "joint1",           # -π to π
            "joint2",           # -1.5 to 1.5  
            "joint3",           # -1.5 to 1.4
        ...
        data = []
        end_link = "end_effector_link"  # 5DOF end effector
        ...
        complete_filename = f"generated_goals_5DOF_{len(df)}_samples01.csv"
        df.to_csv(complete_filename, index=False)
        
        # Save poses only (exactly as you specified for loading)
        pose_df = df[["x", "y", "z", "qx", "qy", "qz", "qw"]]
        pose_filename = "generated_goals_5dof.csv"  # Exact filename you requested
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate 5DOF pose dataset")
    ...
    if hasattr(generator, 'fk_client') and generator.fk_client:
        ...
        if not df.empty:
            print(f"\n🎊 SUCCESS: Generated {len(df)} valid 5DOF samples!")
