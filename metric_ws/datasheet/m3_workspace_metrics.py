...
class WorkspaceAnalyzer:
    ...
    def load_latest_dataset(self):
        ...
        # NEW: Look inside the fk_metrics folder created by Module 2
        search_path = os.path.join("fk_metrics", "*_dataset_*.csv")
        csv_files = glob.glob(search_path)

        if not csv_files:
            self.node.get_logger().error("❌ No dataset found in 'fk_metrics/'. Run Module 2 first.")
            return None
    ...
    def compute_alpha_shape_volume(self, points, alpha=20.0):
        """
    ...
    def analyze(self):
        try:
            ...
            alpha_volume, alpha_mesh = self.compute_alpha_shape_volume(points, alpha=20.0)
