...
class ReachabilityVerifier(Node):
    ...
    def verify_poses(self, csv_5dof, csv_6dof):
        ...
        ik_reliability = (success_5dof / total_5dof * 100) if total_5dof > 0 else 0
        shared_dexterity = (success_6dof / total_6dof * 100) if total_6dof > 0 else 0
        lost_workspace = 100 - shared_dexterity
        ...
        summary_text = (
            ...
            f"------------------------------------------------------------\n"
            f" ⚙️  BASELINE: 5-DOF SELF-REPRODUCTION (Solver Reliability)\n"
            f"------------------------------------------------------------\n"
            f"Total 5-DOF Poses Tested:  {total_5dof}\n"
            f"Successfully Reached:      {success_5dof}\n"
            f"Failed (Singularities):    {fail_5dof}\n"
            f"IK Solver Success Rate:    {ik_reliability:.2f}%\n"
            f"------------------------------------------------------------\n"
            f" 🚀 EXPERIMENTAL: 6-DOF TO 5-DOF DEXTERITY MAPPING\n"
            f"------------------------------------------------------------\n"
            f"Total 6-DOF Poses Tested:  {total_6dof}\n"
            f"Reached by 5-DOF (Shared): {success_6dof}\n"
            f"Exclusive to 6-DOF (Lost): {fail_6dof}\n"
            f"Shared Kinematic Space:    {shared_dexterity:.2f}%\n"
            f"Dexterity Expansion by 6th joint: {lost_workspace:.2f}%\n"
            f"============================================================\n"
        )
        
        # Print and Save Text Report
        print(summary_text)
        report_filename = f"reachability_report_{timestamp}.txt"
        ...
        categories = ['5-DOF Self-Test', '6-DOF Overlap Test']
        success_rates =[ik_reliability, shared_dexterity]
        bars = ax1.bar(categories, success_rates, color=['#4CAF50', '#2196F3'])
        ax1.set_ylim(0, 110)
        ax1.set_ylabel('IK Success Rate / Reachability (%)', fontsize=12, fontweight='bold')
        ax1.set_title('Kinematic Reachability Comparison', fontsize=14, fontweight='bold')
        ...
        # Subplot 2: 3D Mapping of 6-DOF Poses (Showing Lost Dexterity)
        ax2 = fig.add_subplot(122, projection='3d')
        # Filter only 6DOF poses
        df_6_reachable = df6_subset[df6_subset['reachable_by_5dof'] == True]
        df_6_unreachable = df6_subset[df6_subset['reachable_by_5dof'] == False]
        
        ax2.scatter(df_6_reachable['x'], df_6_reachable['y'], df_6_reachable['z'], 
                    c='green', alpha=0.3, s=5, label='Shared Workspace (Reached by 5-DOF)')
        ax2.scatter(df_6_unreachable['x'], df_6_unreachable['y'], df_6_unreachable['z'], 
                    c='red', alpha=0.3, s=5, label='Exclusive Workspace (Unreachable by 5-DOF)')
        
        ax2.set_xlabel('X (m)', fontweight='bold')
        ax2.set_ylabel('Y (m)', fontweight='bold')
        ax2.set_zlabel('Z (m)', fontweight='bold')
        ax2.set_title('6-DOF Dexterity Map (Spatial Distribution)', fontsize=14, fontweight='bold')
        ...
        plt.tight_layout()
        plot_filename = f"reachability_analysis_plot_{timestamp}.png"
        plt.savefig(plot_filename, dpi=300)
        
        # Save Dataset
        out_filename = f"verified_kinematic_overlap_{timestamp}.csv"
