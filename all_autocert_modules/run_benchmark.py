...
# ---> ADDED: IMPORT NEW SCENE MANAGER MODULE <---
from scene_manager import SceneManager


class BenchmarkRunner(Node):

    # ---> ADDED: use_scene parameter <---
    def __init__(self, urdf_path=None, srdf_path=None, use_scene=False):
        ...
        self.use_scene = use_scene # ---> ADDED
    ...
    def run_benchmark(self):
        ...
        try:
            ...
            # ---> ADDED: MODULE 5.5 SCENE MANAGEMENT <---
            # ==========================================
            # MODULE 5.5 : SCENE MANAGEMENT
            # ==========================================
            self.get_logger().info("")
            self.get_logger().info("=======================================================")
            self.get_logger().info("MODULE 5.5: SCENE MANAGEMENT")
            self.get_logger().info("=======================================================")
            try:
                scene_mgr = SceneManager(node=self, base_frame=self.parser.base_link)
                
                if self.use_scene:
                    self.get_logger().info("🧱 --use_scene flag active: Injecting Virtual Obstacles!")
                    scene_mgr.setup_obstacle_scene()
                else:
                    self.get_logger().info("🌌 No --use_scene flag: Ensuring Empty Void Workspace.")
                    scene_mgr.clear_scene()
                    
            except Exception as e:
                self.get_logger().error(f"❌ Module 5.5 (Scene) error: {str(e)}")
...
def main(args=None):
    ...
    # ---> ADDED: Command Line Toggle <---
    parser.add_argument(
        "--use_scene",
        action="store_true",
        help="Inject virtual obstacles into the MoveIt planning scene before path planning"
    )
    ...
    runner = BenchmarkRunner(
        ...
        use_scene=parsed_args.use_scene # ---> ADDED
    )
